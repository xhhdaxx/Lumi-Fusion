...
def process_image(image_path, experiment_config, dce_model, device, 
    ...
    Args:
        ...
        dce_model: Zero-DCE模型（如果需要）
        device: torch设备
        clahe_params: dict, CLAHE参数
        gamma_params: dict, Gamma参数
    ...
    for method in experiment_config['methods']:
        if method == 'zero_dce':
            enhanced = enhancement.apply_zero_dce(enhanced, dce_model, device)
        elif method == 'clahe':
            params = clahe_params or {}
            enhanced = enhancement.apply_clahe(enhanced, **params)
        elif method == 'gamma':
            params = gamma_params or {}
            enhanced = enhancement.apply_gamma(enhanced, **params)
...
def run_experiment(exp_id, test_data_path, output_dir, model_path, device,
    ...
    for idx, image_path in enumerate(all_images):
        ...
        try:
            ...
            # 计算指标（使用原始图像作为参考，因为Zero-DCE数据集通常没有ground truth）
            original_image = np.array(Image.open(image_path))
            if len(original_image.shape) == 3 and original_image.shape[2] == 4:
                original_image = original_image[:, :, :3]
            
            # 计算指标
            metric_results = metrics.calculate_all_metrics(original_image, enhanced_image)
...
def main():
    ...
    parser.add_argument('--model_path', type=str, default='snapshots/Epoch99.pth',
                       help='Path to Zero-DCE model')
    
    # 实验选择
    parser.add_argument('--experiment_ids', type=str, nargs='+', 
                       default=['1', '2', '3', '4', '5', '6', '7'],
                       help='Experiment IDs to run (1-7)')
    ...
    # 其他
    parser.add_argument('--save_results_csv', type=str, default='experiment_results.csv',
                       help='Path to save results CSV file')
    parser.add_argument('--save_results_json', type=str, default='experiment_results.json',
