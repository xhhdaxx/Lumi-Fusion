...
class L_spa(nn.Module):
    ...
    def forward(self, org, enhance):
        ...
        weight_diff = torch.max(torch.FloatTensor([1]).to(self.device) + 10000 * torch.min(
            org_pool - torch.FloatTensor([0.3]).to(self.device), torch.FloatTensor([0]).to(self.device)),
                                torch.FloatTensor([0.5]).to(self.device))
